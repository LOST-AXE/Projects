...
#include "debug.h"
#include "pgm.h"
//code at begininng from class; untouched
...
//throw error if dimensions dont match
  if (static_cast<int>(data.size()) != xdim * ydim)
    throw std::runtime_error (std::format("amount of data in PGM file \"{}\" ({}) does not match the dimensions ({}x{})", filename, data.size(), xdim, ydim));
